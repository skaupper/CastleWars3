/*
 * SDLWrapper.h
 * Copyright (C) 2017 sebastian <sebastian@ARCH-LINUX>
 *
...
#ifdef TEST
#define MANGLE_SDL(name) CONCAT(SDLProvider::getInstance(),"->",name)
#else
...
protected:
    ...
    public:
        ...
        virtual SDL_Window *SDL_CreateWindow(const char *, int, int, int, int, uint32_t);
        ...
        virtual int SDL_SetRenderDrawColor(SDL_Renderer *, uint8_t, uint8_t, uint8_t, uint8_t);
        ...
        virtual SDL_Texture *SDL_CreateTextureFromSurface(SDL_Renderer *, SDL_Surface *);
...
class SDLProvider
{
    ...
    public:
        static SDL *getInstance()  { return instance; }
        static void setInstance(SDL *i) { instance = i; }
};
